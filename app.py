...
def audio_mapping_metadata(obj: AstroObject, environment_name: str):
    """Return the deterministic scientific-to-audio mapping used by the synthesizer."""
    return {
        "base_frequency": "35–125 Hz, derived from pressure on a logarithmic scale and object category; it is an audible carrier, not a measured acoustic frequency.",
        "temperature_mapping": "log10(temperature in kelvin) is normalized over an educational reference range and controls the low-pass cutoff.",
        "pressure_mapping": "log10(pressure in bar) is normalized from 1e-6 to 1e2 bar and contributes to the low-frequency carrier.",
        "intensity_mapping": "User intensity scales synthesized signal amplitude after the physical-parameter mapping.",
        "temporal_modulation": "Environment-specific low-frequency modulation represents modeled temporal variation; it is not a measurement unless explicitly connected to an external dataset.",
        "noise": "Seeded pseudo-random noise is generated from SHA-256-derived object/environment state, making the same configuration reproducible.",
        "sampling": f"Mono PCM WAV at {SAMPLE_RATE:,} Hz, 16-bit integer samples.",
        "object": obj.name,
        "environment": environment_name,
        "provenance": "Physics-based / interpretive deterministic model for the current local registry.",
    }
...
def audio_cache_key(obj: AstroObject, environment_name: str, intensity: float, duration: int) -> str:
    return f"{obj.object_id}|{environment_name}|{float(intensity):.2f}|{int(duration)}"


def get_or_generate_audio(obj: AstroObject, environment_name: str, intensity: float, duration: int):
    key = audio_cache_key(obj, environment_name, intensity, duration)
    if key not in st.session_state.audio_cache:
        st.session_state.audio_cache[key] = synthesize_environment(
...
def render_audio_player(obj: AstroObject, audio: bytes, key: str, environment_name: str):
    st.markdown(f"### Hear the representation")
    st.caption(f"{obj.name} / {environment_name}")
    st.audio(audio, format="audio/wav", start_time=0)
    st.download_button(
        "Download WAV",
        data=audio,
        file_name=f"{obj.object_id}_{environment_name.lower().replace(' ', '_')}.wav",
        mime="audio/wav",
        key=f"download_audio_{key}",
        use_container_width=True,
    )
    st.caption(
        "Use the audio player above to hear the generated representation. "
        "This is synthesized audio, not a literal microphone recording of sound traveling through space."
    )
    ...
    for col, object_id in zip(cols, quick_ids):
        ...
        with col:
            ...
            if st.button("Open object", key=f"home_open_{object_id}", use_container_width=True):
                ...
                st.rerun()
            if st.button("Hear", key=f"home_hear_{object_id}", use_container_width=True):
                with st.spinner(f"Generating the auditory representation of {obj.name}..."):
                    home_audio, home_audio_key = get_or_generate_audio(
                        obj, "Atmosphere", 0.65, 12
                    )
                render_audio_player(obj, home_audio, home_audio_key, "Atmosphere")
...
# ---------- Explore Objects ----------
    ...
    if not results:
        ...
        render_section("Hear this object", "Generate a deterministic auditory representation")
        hear_left, hear_right = st.columns([1, 1])
        with hear_left:
            hear_environment = st.selectbox(
                "Audio environment",
                list(ENVIRONMENTS.keys()),
                index=(list(ENVIRONMENTS.keys()).index("Atmosphere") if "Atmosphere" in ENVIRONMENTS else 0),
                key=f"hear_environment_{obj.object_id}",
            )
        with hear_right:
            hear_duration = st.slider(
                "Duration (seconds)", 5, 30, 12, 1,
                key=f"hear_duration_{obj.object_id}",
            )
        hear_intensity = st.slider(
            "Auditory intensity", 0.1, 1.0, 0.65, 0.05,
            key=f"hear_intensity_{obj.object_id}",
        )
        hear_key = audio_cache_key(obj, hear_environment, hear_intensity, hear_duration)
        if st.button(
            "Hear selected object", type="primary", use_container_width=True,
            key=f"hear_object_{obj.object_id}",
        ):
            with st.spinner("Generating the auditory representation..."):
                audio, _ = get_or_generate_audio(
                    obj, hear_environment, hear_intensity, hear_duration
                )
            st.success("Audio is ready. Use the player below to hear it.")
            render_audio_player(obj, audio, hear_key, hear_environment)
        elif (
            st.session_state.get("last_audio_key") == hear_key
            and st.session_state.get("last_audio_bytes")
        ):
            render_audio_player(
                obj, st.session_state.last_audio_bytes, hear_key, hear_environment
            )
...
# ---------- Audio environments ----------
    ...
    st.session_state._audio_rendered_this_run = False
    ...
    if st.button(
        "Hear / Generate environment",
        type="primary",
        use_container_width=True,
        key="generate_environment_audio",
    ):
        with st.spinner("Synthesizing the auditory representation..."):
            audio, cache_key = get_or_generate_audio(
                obj, environment_name, intensity, duration
            )
        st.success("Audio is ready. Use the player below to hear it.")
        render_audio_player(obj, audio, cache_key, environment_name)
        render_provenance(obj)
        st.session_state._audio_rendered_this_run = True

    current_key = audio_cache_key(obj, environment_name, intensity, duration)
    if (
        st.session_state.get("last_audio_key") == current_key
        and st.session_state.get("last_audio_bytes")
    ):
        if not st.session_state.get("_audio_rendered_this_run", False):
            render_audio_player(
                obj, st.session_state.last_audio_bytes, current_key, environment_name
            )
            render_provenance(obj)

    render_section("Audio mapping", "Deterministic transformation")
    mapping = audio_mapping_metadata(obj, environment_name)
    st.dataframe(
        [{"Parameter": key.replace("_", " ").title(), "Mapping": value} for key, value in mapping.items()],
        use_container_width=True,
        hide_index=True,
    )
