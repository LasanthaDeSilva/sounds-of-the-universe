...
GEMINI_SONIFICATION_MODEL = "gemini-2.5-flash"
...
class AudioResult:
    ...
    explanation: str
    cache_key: str
    psychoacoustic_config: dict[str, Any] = field(default_factory=dict)
...
# ---------------------------------------------------------------------------
# Search-and-Sonify ingestion layer
# ---------------------------------------------------------------------------

def fetch_astronomical_data(query: str) -> dict[str, Any]:
    """Resolve *query* against the live astronomy services already used by the app.

    The function deliberately returns raw/structured evidence rather than asking
    Gemini to invent missing astronomy.  SIMBAD/CDS supplies identity and
    coordinates, the NASA Exoplanet Archive supplies exoplanet/host parameters,
    and JPL Horizons supplies Solar-System context when applicable.  Values that
    are not actually available remain ``None``.

    Data flow:
        user query -> astronomical resolvers -> normalized evidence -> Gemini
        mapping layer -> deterministic NumPy oscillator engine.
    """
    query = (query or "").strip()
    if not query:
        raise ValueError("An astronomical object name or catalogue identifier is required.")

    profile = build_profile(query)

    def first_property(*keys: str) -> Any:
        for key in keys:
    ...
    # Keep expansion velocity separate from generic radial velocity: they are
    # scientifically different quantities and must not be silently conflated.
    normalized = {
        "object_name": profile.canonical_name,
        "object_type": profile.object_type,
        "aliases": profile.aliases[:50],
        "coordinates": {"ra_deg": profile.ra_deg, "dec_deg": profile.dec_deg},
        "metrics": {
            "mass": first_property("pl_bmasse", "pl_bmassj", "st_mass", "mass_msun", "mass_kg"),
            "radius": first_property("pl_rade", "pl_radj", "st_rad", "radius_km", "radius_rsun"),
            "temperature": first_property("pl_eqt", "st_teff", "temperature_k", "temp_k"),
            "expansion_velocity": first_property("expansion_velocity_kms", "expansion_velocity", "expansion_speed_kms"),
        },
        "properties": profile.properties.copy(),
        "sources": [asdict(source) for source in profile.sources],
        "retrieved_at": profile.retrieved_at,
        "data_quality": profile.data_quality,
        "retrieval_errors": profile.retrieval_errors[:],
    }
    return normalized
...
def _validate_psychoacoustic_config(raw: Any, object_name: str = "Astronomical object") -> dict[str, Any]:
    """Validate Gemini's JSON so untrusted model output never controls arbitrary DSP."""
    if not isinstance(raw, dict):
        raise ValueError("Gemini did not return a JSON object.")

    mapping = raw.get("audio_mapping_parameters")
    if not isinstance(mapping, dict):
        raise ValueError("Gemini JSON is missing audio_mapping_parameters.")

    bounds = {
        "sub_bass_frequency_hz": (20.0, 50.0),
        "time_dilation_lfo_hz": (0.02, 1.5),
        "granular_density": (0.0, 1.0),
        "granular_grain_size_ms": (4.0, 80.0),
        "acoustic_grit_factor": (0.0, 1.0),
        "ambient_noise_smoothing": (20.0, 1000.0),
        "atmospheric_dampening_coefficient": (0.0, 1.0),
        "fm_modulation_index": (0.0, 12.0),
    }
    clean: dict[str, Any] = {
        "object_name": str(raw.get("object_name") or object_name)[:240],
        "psychological_concept": str(raw.get("psychological_concept") or "Scientific-scale auditory representation")[:500],
        "audio_mapping_parameters": {},
    }
    for key, (low, high) in bounds.items():
        value = _safe_float(mapping.get(key), (low + high) / 2.0)
        clean["audio_mapping_parameters"][key] = clamp(value, low, high)
    return clean


def _fallback_psychoacoustic_config(scraped_data: dict[str, Any]) -> dict[str, Any]:
    """Deterministic fallback used when Gemini is unavailable or returns invalid JSON."""
    object_type = str(scraped_data.get("object_type", "")).lower()
    metrics = scraped_data.get("metrics", {}) or {}
    temp = metrics.get("temperature")
    temp_norm = normalized_number(temp, 50, 50000, 0.5) if temp is not None else 0.5
    dense = any(token in object_type for token in ("black hole", "neutron", "pulsar", "quasar", "supernova"))
    cold = any(token in object_type for token in ("nebula", "molecular cloud", "comet"))
    return _validate_psychoacoustic_config({
        "object_name": scraped_data.get("object_name", "Astronomical object"),
        "psychological_concept": (
            "Dense, slow-scale gravitational presence" if dense else
            "Cold, spacious drift" if cold else
            "Measured-parameter scientific scale translated into controlled sound"
        ),
        "audio_mapping_parameters": {
            "sub_bass_frequency_hz": 30.0 + 12.0 * temp_norm,
            "time_dilation_lfo_hz": 0.12 if dense else 0.28,
            "granular_density": 0.78 if "supernova" in object_type else 0.38,
            "granular_grain_size_ms": 18.0 if "supernova" in object_type else 30.0,
            "acoustic_grit_factor": 0.62 if "supernova" in object_type else 0.18,
            "ambient_noise_smoothing": 240.0 if cold else 150.0,
            "atmospheric_dampening_coefficient": 0.72 if "planet" in object_type else 0.22,
            "fm_modulation_index": 7.0 if "star" in object_type else 4.0,
        },
    }, str(scraped_data.get("object_name", "Astronomical object")))


def generate_psychoacoustic_config(scraped_data: dict[str, Any]) -> dict[str, Any]:
    """Send verified astronomy evidence to Gemini and return strict DSP parameters.

    Gemini is constrained to map supplied evidence into an explicit acoustic
    design schema. It cannot supply missing astronomical measurements, generate
    waveform code, or select unbounded DSP values. The deterministic NumPy engine
    remains responsible for the actual audio.
    """
    fallback = _fallback_psychoacoustic_config(scraped_data)
    client = get_gemini_client()
    if client is None or types is None:
        return fallback

    system_instruction = """
You are the Somatic Sound Designer for Sounds of the Universe.
Your job is NOT to invent astronomy and NOT to generate audio.
Translate only the supplied astronomical evidence into a controlled psychoacoustic
DESIGN SCHEMA for a deterministic Python sonification engine.

Use the archetypes Crushing Weight, Kinetic Violence, Cold Immensity, and
Atmospheric Pressure only as design concepts, not as scientifically validated
claims about human psychology.

STRICT RULES:
1. Use only supplied measurements, classifications, coordinates, provenance, and
   object type. Never invent a missing mass, radius, temperature, velocity,
   atmosphere, composition, gravitational-wave measurement, or other observation.
2. Return ONLY valid JSON. No markdown fences. No commentary.
3. Return exactly the requested top-level keys and exactly the eight numeric
   audio_mapping_parameters.
4. Keep values within these safe ranges:
   sub_bass_frequency_hz: 20-50
   time_dilation_lfo_hz: 0.02-1.5
   granular_density: 0-1
   granular_grain_size_ms: 4-80
   acoustic_grit_factor: 0-1
   ambient_noise_smoothing: 20-1000
   atmospheric_dampening_coefficient: 0-1
   fm_modulation_index: 0-12
5. The numbers are mapping controls, not measured physical quantities.
6. Prefer direct scientific evidence over object stereotypes. If evidence is sparse,
   make the mapping conservative and say so in psychological_concept.
...
Required JSON shape:
{
  ...
  "psychological_concept": "...",
  "audio_mapping_parameters": {
    "sub_bass_frequency_hz": 32,
    "time_dilation_lfo_hz": 0.3,
    "granular_density": 0.85,
    "granular_grain_size_ms": 15,
    "acoustic_grit_factor": 0.7,
    "ambient_noise_smoothing": 180,
    "atmospheric_dampening_coefficient": 0.15,
    "fm_modulation_index": 8.0
  }
}
"""
    prompt = "ASTRONOMICAL EVIDENCE:\n" + json.dumps(scraped_data, ensure_ascii=False, indent=2, default=str)
    try:
        response = client.models.generate_content(
            model=GEMINI_SONIFICATION_MODEL,
            contents=prompt,
        ...
        if text:
            return _validate_psychoacoustic_config(json.loads(text), scraped_data.get("object_name", "Astronomical object"))
    except Exception:
        pass
    return fallback
...
def synthesize_audio(
    ...
    duration: float = DEFAULT_DURATION,
    intensity: float = 0.65,
    psychoacoustic_config: Optional[dict[str, Any]] = None,
    source_data: Optional[dict[str, Any]] = None,
) -> AudioResult:
    """Synthesize deterministic tactile/psychoacoustic audio from Gemini's config.

    The model supplies bounded mapping numbers; this function alone turns those
    numbers into oscillators, LFOs, grains, smoothed noise and FM synthesis.
    A tanh saturation stage provides a soft nonlinear peak limiter before the
    native ``wave`` module writes a standard PCM WAV byte string.
    """
    if not psychoacoustic_config:
        psychoacoustic_config = _fallback_psychoacoustic_config(source_data or {
            "object_name": profile.canonical_name,
            "object_type": profile.object_type,
            "metrics": profile.properties,
        })
    config = _validate_psychoacoustic_config(psychoacoustic_config, profile.canonical_name)
    p = config["audio_mapping_parameters"]
    duration = clamp(float(duration), 4.0, 30.0)
    intensity = clamp(float(intensity), 0.15, 1.0)
    sr = AUDIO_SAMPLE_RATE
    n = int(duration * sr)
    t = np.arange(n, dtype=np.float64) / sr
    seed_payload = {"profile": asdict(profile), "config": config, "duration": duration, "intensity": intensity}
    seed = int.from_bytes(hashlib.sha256(json.dumps(seed_payload, sort_keys=True, default=str).encode()).digest()[:8], "big") & 0xFFFFFFFF
    rng = np.random.default_rng(seed)
    audio = np.zeros(n, dtype=np.float64)
    mapping: list[dict[str, Any]] = []

    # 1. Crushing Weight: clean 20-50 Hz sine. It is intentionally not boosted
    # beyond a conservative level; tactile sensation varies by playback hardware.
    sub = p["sub_bass_frequency_hz"]
    audio += 0.34 * np.sin(2 * np.pi * sub * t)
    mapping.append({"layer": "Crushing Weight", "parameter": "sub_bass_frequency_hz", "value": sub,
                    "description": "Controlled 20-50 Hz sine carrier; perceptual tactile impact depends on the playback system."})

    # 2. Cold Immensity: slow LFO + heavily smoothed noise.
    lfo_hz = p["time_dilation_lfo_hz"]
    lfo = 0.72 + 0.28 * (0.5 + 0.5 * np.sin(2 * np.pi * lfo_hz * t))
    smoothing = int(clamp(p["ambient_noise_smoothing"], 20, 1000))
    noise = rng.normal(0.0, 1.0, n)
    kernel = np.ones(max(2, smoothing), dtype=np.float64)
    ambient = np.convolve(noise, kernel / kernel.size, mode="same")
    ambient /= max(float(np.max(np.abs(ambient))), 1e-9)
    audio += ambient * (0.055 + 0.06 * p["ambient_noise_smoothing"] / 1000.0) * lfo
    mapping.append({"layer": "Cold Immensity", "parameter": "ambient_noise_smoothing", "value": smoothing,
                    "description": "Long moving-average smoothing produces a slow, spacious noise bed."})
    mapping.append({"layer": "Time Dilation", "parameter": "time_dilation_lfo_hz", "value": lfo_hz,
                    "description": "Slow LFO changes the amplitude envelope rather than claiming to reproduce literal spacetime."})

    # 3. Kinetic Violence: deterministic granular bursts and high-frequency grit.
    density = p["granular_density"]
    grain_ms = p["granular_grain_size_ms"]
    grain_n = max(8, int(sr * grain_ms / 1000.0))
    expected_grains = max(1, int(duration * (2.0 + 22.0 * density)))
    for _ in range(expected_grains):
        start = int(rng.integers(0, max(1, n - grain_n)))
        freq = float(rng.uniform(700.0, 5200.0))
        length = min(grain_n, n - start)
        gt = np.arange(length) / sr
        grain_env = np.sin(np.pi * np.arange(length) / max(length - 1, 1)) ** 2
        grain = np.sin(2 * np.pi * freq * gt) * grain_env
        audio[start:start + length] += grain * (0.012 + 0.05 * density)
    grit = rng.normal(0.0, 1.0, n)
    # A simple differentiator emphasizes high-frequency edge without requiring
    # scipy; it remains deterministic and bounded.
    grit = np.concatenate(([0.0], np.diff(grit)))
    grit /= max(float(np.max(np.abs(grit))), 1e-9)
    audio += grit * (0.018 * p["acoustic_grit_factor"])
    mapping.append({"layer": "Kinetic Violence", "parameter": "granular_density/grain_size", "value": [density, grain_ms],
                    "description": "Deterministic micro-grains and controlled high-frequency grit create transient motion."})

    # 4. Atmospheric Pressure: FM synthesis whose carrier/modulator relationship
    # is filtered by a rolling average. This is a sound-design model, not a claim
    # that the atmosphere itself emits this waveform.
    damp = p["atmospheric_dampening_coefficient"]
    fm_index = p["fm_modulation_index"]
    carrier = 90.0 + 240.0 * (1.0 - damp)
    mod_freq = 0.35 + 3.5 * (1.0 - damp)
    fm = np.sin(2 * np.pi * carrier * t + fm_index * np.sin(2 * np.pi * mod_freq * t))
    roll = max(8, int(120 + 900 * damp))
    kernel2 = np.ones(roll, dtype=np.float64) / roll
    filtered = np.convolve(fm, kernel2, mode="same")
    filtered /= max(float(np.max(np.abs(filtered))), 1e-9)
    audio += filtered * (0.10 + 0.12 * (1.0 - damp))
    mapping.append({"layer": "Atmospheric Pressure", "parameter": "dampening/fm_index", "value": [damp, fm_index],
                    "description": "FM sidebands plus rolling-average filtering create dense or charged atmospheric texture."})

    # Smooth start/end, controlled intensity, and tanh soft-knee saturation.
    attack = np.minimum(1.0, t / 0.12)
    release = np.minimum(1.0, (duration - t) / 0.30)
    envelope = np.clip(np.minimum(attack, release), 0.0, 1.0)
    audio *= envelope
    audio = np.tanh(audio * (0.82 + 0.48 * intensity))

    explanation = (
        f"The waveform is a deterministic scientific sonification design for {profile.canonical_name}. "
        "The low-frequency carrier supplies controlled tactile-scale energy, while slow modulation, "
        "granular transients, smoothed noise, and FM/filtering create the object's mapped sonic character. "
        "These are auditory mappings of scientific evidence, not literal sounds recorded in space."
    )
    cache_key = hashlib.sha256(json.dumps(seed_payload, sort_keys=True, default=str).encode()).hexdigest()
    return AudioResult(
        wav_bytes=make_wav(audio, sr), duration=duration, sample_rate=sr, seed=seed,
        mode="Deterministic psychoacoustic scientific sonification", mapping=mapping,
        explanation=explanation, cache_key=cache_key, psychoacoustic_config=config,
        source_data=source_data or {},
    )

def prepare_object(query: str) -> ObjectProfile:
    """Run the complete Search -> Evidence -> Gemini -> DSP -> WAV pipeline."""
    scraped_data = fetch_astronomical_data(query)
    profile = build_profile(query)
    profile.audio_basis = choose_audio_basis(profile)
    psycho_config = generate_psychoacoustic_config(scraped_data)
    profile.audio_mode = "Deterministic psychoacoustic scientific sonification"
    profile.audio_explanation = str(psycho_config.get("psychological_concept", "Scientific auditory mapping"))
    profile.limitations = [
        "Psychological archetypes are sound-design concepts, not validated diagnoses or universal perceptual responses.",
        "Generated audio is a sonification/modeled representation, not literal sound recorded from space.",
    ]
    st.session_state.scientific_data = scraped_data
    st.session_state.psychoacoustic_config = psycho_config
    st.session_state.profile = profile
    st.session_state.audio = synthesize_audio(profile, psychoacoustic_config=psycho_config, source_data=scraped_data)
...
PAGES = ["Home", "Object", "Audio Lab", "Favorites", "Data / Export", "Scientific Method", "About"]
...
if "scientific_data" not in st.session_state: st.session_state.scientific_data = {}
if "psychoacoustic_config" not in st.session_state: st.session_state.psychoacoustic_config = {}
...
# ---------------------------------------------------------------------------
    ...
    if not st.session_state.profile:
        ...
        psycho = st.session_state.get("psychoacoustic_config", {}) or {}
        concept = psycho.get("psychological_concept", "Scientific auditory mapping")
        st.markdown('<div class="su-section">Auditory concept</div>', unsafe_allow_html=True)
        st.markdown(
            f'<div class="su-dark"><div class="su-label">DESIGN CONCEPT</div><div style="font-size:2rem;line-height:1.15;margin-top:.45rem">{concept}</div></div>',
            unsafe_allow_html=True,
        )
...
""",
        ...
        if st.session_state.audio is None:
            with st.spinner("Building deterministic scientific audio…"):
                st.session_state.audio = synthesize_audio(profile, psychoacoustic_config=st.session_state.get("psychoacoustic_config"), source_data=st.session_state.get("scientific_data"))
        ...
        st.download_button("Download WAV", data=st.session_state.audio.wav_bytes, file_name=f"{re.sub(r"[^A-Za-z0-9._-]+", "_", profile.canonical_name)}_scientific_sonification.wav", mime="audio/wav", use_container_width=True)
...
# ---------------------------------------------------------------------------
    ...
    if not st.session_state.profile:
        ...
        if st.button("Regenerate deterministic audio", type="primary", use_container_width=True):
            st.session_state.audio = synthesize_audio(profile, duration, intensity, st.session_state.get("psychoacoustic_config"), st.session_state.get("scientific_data"))

        audio = st.session_state.audio or synthesize_audio(profile, duration, intensity, st.session_state.get("psychoacoustic_config"), st.session_state.get("scientific_data"))
        ...
        st.markdown('<div class="su-section">Gemini mapping schema</div>', unsafe_allow_html=True)
        st.json(audio.psychoacoustic_config, expanded=True)
        ...
        st.download_button(
            ...
            file_name=f"{re.sub(r'[^A-Za-z0-9._-]+', '_', profile.canonical_name)}_scientific_sonification.wav",
            mime="audio/wav",
