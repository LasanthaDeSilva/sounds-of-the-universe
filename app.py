...
def page_search() -> None:
    st.markdown('<div class="su-eyebrow">Universal search</div>', unsafe_allow_html=True)
    st.markdown("# Search the universe")
    st.markdown(
        "Search for any astronomical object catalogued on Wikidata -- a specific star, "
        "exoplanet, asteroid, comet, galaxy, or anything else with a name. This queries "
        "Wikidata and Wikipedia live over the internet, extracts whatever structured "
        "physical properties actually exist for it, and builds that object's auditory "
        "representation from those real properties using the same deterministic engine as "
        "the curated catalogue -- never a generic effect with the name swapped in."
    )
    if requests is None:
        st.warning("The 'requests' library is not installed in this environment, so live search cannot run.")

    query = st.text_input(
        "Object name", value="", key="universal_search_query",
        placeholder="e.g. Betelgeuse, TRAPPIST-1e, Andromeda Galaxy, Halley's Comet, Ceres",
    )
    if st.button("Search", key="universal_search_button") and query.strip():
        with st.spinner(f"Searching the astronomical database for '{query.strip()}'..."):
            profile, alternates, error = lookup_and_build_profile(query.strip())
        st.session_state["last_search_alternates"] = alternates
        if error:
            st.warning(error)
        else:
            st.session_state["dynamic_objects"][profile.object_id] = profile
            st.session_state["selected_object"] = profile.object_id
            st.success(f"Found: {profile.object_name} ({OBJECT_TYPE_LABELS.get(profile.object_type, profile.object_type)})")

    alternates = st.session_state.get("last_search_alternates", [])
    if alternates:
        st.caption("Other possible matches:")
        cols = st.columns(min(len(alternates), 4))
        for col, alt in zip(cols, alternates[:4]):
            with col:
                if st.button(alt["label"], key=f"alt_{alt['id']}", help=alt.get("description", "")):
                    with st.spinner(f"Loading {alt['label']}..."):
                        alt_profile = build_profile_from_qid(alt["id"], alt["label"])
                    if alt_profile is not None:
                        st.session_state["dynamic_objects"][alt_profile.object_id] = alt_profile
                        st.session_state["selected_object"] = alt_profile.object_id
                        st.rerun()

    st.markdown('<hr class="su-divider">', unsafe_allow_html=True)
    dynamic = st.session_state["dynamic_objects"]
    selected = st.session_state.get("selected_object")
    if selected in dynamic:
        render_object_profile(dynamic[selected])
    elif dynamic:
        render_object_profile(list(dynamic.values())[-1])
    else:
        st.caption(
            "Search results will appear here, and will also show up in Explore objects, the "
            "Audio laboratory, Compare objects, Favorites, and Data and export for the rest of "
            "this session."
        )
