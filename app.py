...
# ---------------------------------------------------------------------------
# Somatic sound-design schema (primary AI-driven audio pipeline)
# ---------------------------------------------------------------------------
#
# This schema and generate_somatic_profile() replace the previous rule-based
# per-object-type audio engine as the primary sound-generation path. There is
# intentionally no procedural/rule-based audio fallback here: if Gemini is
# unavailable or the call fails, generate_somatic_profile() raises, and the
# UI layer (ensure_somatic_audio(), below) surfaces that as a clear message
# instead of generating a substitute soundscape.

if BaseModel is not None:
    class PsychoacousticParameters(BaseModel):
        sub_bass_frequency_hz: float = Field(
            description="20-50Hz frequency mapping object weight/gravity for deep physical somatic vibration."
        )
        time_dilation_lfo_hz: float = Field(
            description="0.1-1.5Hz oscillation rate simulating relativistic pulling and spatial warping."
        )
        granular_density: float = Field(
            description="0.0-1.0 matrix defining micro-shards of kinetic debris, explosion shockwaves, or grit."
        )
        granular_grain_size_ms: float = Field(
            description="10-50ms grain frame size controlling the texture sharpness."
        )
        acoustic_grit_factor: float = Field(
            description="0.0-1.0 clipping distortion emulation to give elements a tangible, rough material feel."
        )
        ambient_noise_smoothing: float = Field(
            description="50-250 convolution smoothing value to control expansive, cold, gaseous fog."
        )
        atmospheric_dampening_coefficient: float = Field(
            description="0.0-1.0 high-cut filter roll-off simulating dense, trapping pressure environments."
        )
        fm_modulation_index: float = Field(
            description="0.0-25.0 phase distortion rate indicating violent weather or highly charged electromagnetic fields."
        )

    class SomaticSpaceConfig(BaseModel):
        object_name: str
        psychological_concept: str = Field(
            description="A 1-sentence description explaining how the listener's body and mind should interpret the feeling of this sound environment."
        )
        audio_mapping_parameters: PsychoacousticParameters
else:
    PsychoacousticParameters = None  # type: ignore
    SomaticSpaceConfig = None  # type: ignore


SOMATIC_SYSTEM_INSTRUCTION = (
    "You are a Cognitive Psychoacoustic and Somatic Sound Designer for blind and visually impaired users. "
    "Analyze the provided astronomical metrics and calculate the precise acoustic vectors required to "
    "trigger an accurate psychological and bodily 'feeling' of that specific cosmic environment in the listener. "
    "Do not invent data properties, but completely scale the synthesis schema to represent the actual scale, "
    "danger, density, pressure, or coldness of the object."
)


def generate_somatic_profile(profile: ObjectProfile) -> "SomaticSpaceConfig":
    """
    Primary AI sound-design step. Raises ValueError/RuntimeError on any
    failure (no Gemini client, no pydantic installed, malformed response) so
    callers can decide how to surface that — there is deliberately no
    rule-based numerical fallback baked into this function.
    """
    if SomaticSpaceConfig is None:
        raise RuntimeError("pydantic is not installed; the somatic sound-design schema is unavailable.")

    client = get_gemini_client()
    if client is None:
        raise ValueError("Gemini API client failed to initialize.")
    ...
    response = client.models.generate_content(
        model=model_to_use,
        contents=f"Generate the somatic sound config for this object data: {json.dumps(payload, ensure_ascii=False, default=str)}",
        config=types.GenerateContentConfig(
            system_instruction=SOMATIC_SYSTEM_INSTRUCTION,
            response_mime_type="application/json",
            response_schema=SomaticSpaceConfig,
            temperature=0.2,
        ),
    )
    ...
    # Some SDK versions/models don't populate .parsed even with a schema; fall
    # back to validating the raw text against the same schema rather than
    # silently returning nothing.
    text = (getattr(response, "text", "") or "").strip()
    text = re.sub(r"^```(?:json)?|```$", "", text, flags=re.MULTILINE).strip()
    if not text:
        raise ValueError("Gemini returned an empty response for the somatic sound profile.")
    return SomaticSpaceConfig.model_validate_json(text)
...
def _render_somatic_waveform(params: dict, duration: float, seed_material: str) -> tuple:
    """
    Shared NumPy oscillator/noise/FM rendering core used by both the primary
    AI-driven audio pipeline (synthesize_audio, config-generated by Gemini)
    and the Search-and-Sonify tactile pipeline (synthesize_tactile_audio).
    Returns (mix, sample_rate, clamped_duration) BEFORE the final saturation
    stage, so callers can apply their own drive/limiter.
    """
    ...
    # Time dilation / relativistic pulling: a crawling LFO that stretches gain
    # over a slow cycle, to feel like space/time stretching.
    ...
    # grit, simulating a violent, expanding explosion or shockwave fragments.
    ...
    # Cold, gaseous fog: heavily smoothed ambient noise sweeps for freezing
    # drift through an endless, expansive void.
    ...
    # Suffocating, high-pressure atmosphere vs. an electric, stormy field: FM
    # synthesis loop with a rolling-average digital low-pass (moving-average)
    # filter standing in for atmospheric dampening / high-cut roll-off.
...
def synthesize_audio(
    profile: ObjectProfile,
    config: "SomaticSpaceConfig",
    duration: float = DEFAULT_DURATION,
    intensity: float = 0.65,
) -> AudioResult:
    """
    Primary audio-generation function. Renders the AI-generated somatic sound
    profile (from generate_somatic_profile) into a WAV. This replaces the
    previous rule-based, per-object-type procedural engine: there is no
    numerical fallback here, so a valid `config` from Gemini is required.
    """
    duration = clamp(float(duration), 4.0, 30.0)
    intensity = clamp(float(intensity), 0.15, 1.0)
    params = config.audio_mapping_parameters.model_dump()
    seed_material = json.dumps(
        {"name": profile.canonical_name, "params": params}, sort_keys=True, default=str
    )

    mix, sr, duration = _render_somatic_waveform(params, duration, seed_material)

    # Soft-knee hyperbolic tangent saturation limiter: prevents clipping and
    # maximizes somatic low-end tactile rumbling, prioritizing safety for BVI
    # headphone playback. The "Listening intensity" control drives it.
    mix = np.tanh(mix * (0.9 + intensity * 0.9))

    mapping = [{"parameter": key, "value": value} for key, value in params.items()]
    seed = int(hashlib.sha256(seed_material.encode("utf-8")).hexdigest()[:8], 16)
    cache_key = hashlib.sha256(f"{seed_material}|{duration}|{intensity}".encode("utf-8")).hexdigest()

    return AudioResult(
        wav_bytes=make_wav(mix, sr),
        duration=duration,
        sample_rate=sr,
        seed=seed,
        mode="AI somatic sound design (Gemini)",
        mapping=mapping,
        explanation=config.psychological_concept,
        cache_key=cache_key,
    )
...
def synthesize_tactile_audio(plan: dict[str, Any], duration: float = 14.0) -> dict[str, Any]:
    ...
    its deterministic fallback) via the shared _render_somatic_waveform core.
    This is the standalone Search-and-Sonify page's engine.
    """
    params = dict(plan.get("audio_mapping_parameters") or {})
    seed_material = json.dumps(params, sort_keys=True, default=str)
    mix, sr, duration = _render_somatic_waveform(params, duration, seed_material)

    # Soft-knee hyperbolic tangent saturation limiter to prevent clipping and
    # maximize somatic low-end impact.
    mix = np.tanh(mix * 1.4)

    return {"wav_bytes": make_wav(mix, sr), "duration": duration, "sample_rate": sr}


def ensure_somatic_audio(
    profile: ObjectProfile,
    duration: float = DEFAULT_DURATION,
    intensity: float = 0.65,
    ...
    """
    Ensures st.session_state.audio reflects a Gemini-generated somatic sound
    profile for `profile`. Unlike the previous engine, there is no procedural
    audio fallback: if Gemini is unavailable or fails, no audio is produced
    and the error is recorded in st.session_state.somatic_error for the UI to
    display, instead of letting an unhandled exception crash the page or
    silently substituting a rule-based sound.

    A cached config is only re-requested from Gemini when force_refresh=True
    or none exists yet; otherwise duration/intensity changes only re-run the
    (cheap, local) NumPy rendering step, so slider adjustments don't spam the
    API.
    """
    config = None if force_refresh else st.session_state.get("somatic_config")
    if config is None:
        try:
            config = generate_somatic_profile(profile)
        ...
        profile.audio_mode = "AI somatic sound design (Gemini)"
        profile.audio_explanation = config.psychological_concept

    st.session_state.audio = synthesize_audio(profile, config, duration, intensity)


def prepare_object(query: str) -> ObjectProfile:
    """Resolve, enrich, and sonify one selected object in one workflow."""
    profile = build_profile(query)
...
""",
    ...
    with c3:
        st.markdown("### Sonify")
        st.write("Convert defensible data into an AI-generated somatic auditory representation and explain every mapping.")
...
""",
        ...
        if st.session_state.audio is None and st.session_state.somatic_config is None and not st.session_state.somatic_error:
            with st.spinner("Requesting a live somatic sound profile from Gemini…"):
                ensure_somatic_audio(profile)

        st.markdown('<div class="su-section">Somatic audio</div>', unsafe_allow_html=True)
        ...
        if st.button("Regenerate somatic audio", use_container_width=True):
            with st.spinner("Requesting a fresh somatic sound profile from Gemini…"):
...
# ---------------------------------------------------------------------------
    ...
    if not st.session_state.profile:
        ...
        if st.button("Regenerate somatic profile from Gemini", type="primary", use_container_width=True):
            ...
            with st.spinner("Requesting a somatic sound profile from Gemini…"):
                ensure_somatic_audio(profile, duration, intensity)
...
# ---------------------------------------------------------------------------
    ...
    sections = [
        (
            "AI-generated somatic representation (primary mode)",
            "The primary audio engine sends the resolved scientific profile to a Gemini model, which returns "
            "a structured sound-design configuration (a one-sentence psychological concept plus numeric audio "
            "parameters). The Python audio engine renders exactly those parameters; Gemini is instructed not to "
            "invent new physical measurements, but its output is generative rather than deterministic, so the "
            "same object can sound different across requests."
        ),
        (
