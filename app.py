...
.su-topbar { position: sticky; top: 0; z-index: 999; background: rgba(247,247,245,.98); border: 1px solid var(--line); padding: .65rem .75rem; backdrop-filter: blur(10px); }
.su-brand { display:flex; flex-direction:column; gap:.15rem; line-height:1.05; white-space:nowrap; }
.su-brand strong { font-size:.98rem; letter-spacing:-.02em; }
.su-brand span { font-family:"DM Mono",monospace; font-size:.55rem; letter-spacing:.1em; color:var(--muted); }
.su-header-rule { height:.45rem; }
.su-topbar button { min-height:2.35rem; font-size:.72rem; white-space:nowrap; }
.su-topbar [data-testid="stSelectbox"] label { display:none; }
.su-topbar [data-baseweb="select"] > div { min-height:2.35rem; border-radius:0; border-color:var(--line); background:#fff; color:#101216; }
...
brand, nav, model_col = st.columns([2.1, 6.9, 2.4], vertical_alignment="center")
with brand: st.markdown(f'<div class="su-brand"><strong>{APP_NAME}</strong><span>ASTRONOMICAL AUDIO EXPLORATION</span></div>', unsafe_allow_html=True)
with nav:
    nav_cols = st.columns(len(PAGES), gap="small")
    for col, page_name in zip(nav_cols, PAGES):
        with col:
            if st.button(page_name, key=f"topnav_{page_name}", use_container_width=True, type="primary" if st.session_state.page == page_name else "secondary"):
                st.session_state.page = page_name; st.rerun()
with model_col:
    selected_model = st.selectbox("Gemini model", list(MODEL_OPTIONS), index=list(MODEL_OPTIONS).index(st.session_state.gemini_model) if st.session_state.gemini_model in MODEL_OPTIONS else 0, format_func=lambda x: MODEL_OPTIONS[x], label_visibility="collapsed")
    st.session_state.gemini_model = selected_model
st.markdown('</div><div class="su-header-rule"></div>', unsafe_allow_html=True)
